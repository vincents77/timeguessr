...
export default function GuessResultModal({
  ...
  const renderProgressLabel = () => {
    const currentRound = accepted ? sessionProgress.played : sessionProgress.played + 1;
    const isFinalRound = targetEvents && currentRound === targetEvents;

    console.log({ currentRound, targetEvents, isFinalRound, sessionProgress });
  
    return (
      <div className="text-center space-y-1 mb-2">
        {isFinalRound && (
          <div className="bg-purple-200 text-purple-900 text-xs font-bold px-3 py-1 rounded w-fit mx-auto uppercase tracking-wide shadow-md border border-purple-400">
            Final Event!
          </div>
        )}
        <div className="text-sm text-gray-700 font-semibold bg-gray-100 px-3 py-1 rounded w-fit mx-auto">
          {targetEvents
            ? `Round ${currentRound} of ${sessionProgress.total}`
            : `Round ${currentRound}`}
        </div>
      </div>
    );
